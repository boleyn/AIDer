...
- 使用与问题相同的语言回答。
- 当回答中包含链接时，该链接必须用"**"包裹。例如：访问 **https://example.com** 了解更多信息。
...
export const getHistorySummaryPrompt = (version?: string) => {
  const promptMap = {
    ['4.9.7']: `你是对话记忆压缩助手，请将历史对话压缩为可长期记忆的摘要。

要求：
- 只保留关键信息：用户目标/偏好/约束、重要事实、结论、待办
- 删除寒暄、重复、调试过程
- 不要编造或推断
- 输出精炼要点，使用与对话相同的语言
...
export const getHistorySummarySystemPrompt = (version?: string) => {
  const promptMap = {
    ['4.9.7']: `以下是对话的中期记忆摘要，可作为背景参考（无需复述给用户）：
<Summary>
{{summary}}
</Summary>`
  };
