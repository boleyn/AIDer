...
const GPT2Chat = {
  ...
  developer: ChatRoleEnum.System
} as const satisfies Record<string, ChatRoleEnum>;
...
export const chats2GPTMessages = ({
  ...
  messages.forEach((item) => {
    ...
    if (item.obj === ChatRoleEnum.System) {
      ...
      let pendingAssistant: ChatCompletionMessageParam | null = null;
      const flushPending = () => {
        if (pendingAssistant) {
          aiResults.push(pendingAssistant);
      ...
      item.value.forEach((value) => {
        ...
        if (value.type === ChatItemValueTypeEnum.reasoning) {
          const reasoning = value.reasoning?.content || '';
          if (!pendingAssistant) {
            pendingAssistant = {
              dataId,
              role: ChatCompletionRequestMessageRoleEnum.Assistant,
              ...
              // @ts-ignore - some providers expect reasoning_content
              reasoning_content: reasoning
            };
          } else {
            const currentReasoning = (pendingAssistant as any).reasoning_text || '';
            const mergedReasoning = currentReasoning
              ? `${currentReasoning}\n${reasoning}`
              : reasoning;
            (pendingAssistant as any).reasoning_text = mergedReasoning;
            (pendingAssistant as any).reasoning_content = mergedReasoning;
          }
          return;
        }

        if (value.type === ChatItemValueTypeEnum.text && typeof value.text?.content === 'string') {
          ...
          if (!pendingAssistant) {
            ...
            const prevContent =
              typeof pendingAssistant.content === 'string' ? pendingAssistant.content : '';
            pendingAssistant.content = prevContent + value.text.content;
          }
          return;
        }

        if (value.type === ChatItemValueTypeEnum.interactive) {
          flushPending();
          aiResults.push({
            dataId,
            role: ChatCompletionRequestMessageRoleEnum.Assistant,
          ...
          if (item.tool_calls && reserveTool) {
            ...
            value.push({
              ...
              tools: toolCalls.map((tool) => {
                ...
                const toolInfo = getToolInfo?.(toolFn.name);
                const toolParams =
                  typeof toolFn.arguments === 'string'
                    ? toolFn.arguments
                    : toolFn.arguments
                      ? JSON.stringify(toolFn.arguments)
                      : '';
