...
export const concatHistories = (histories1: ChatItemType[], histories2: ChatItemType[]) => {
  ...
  const systemHistories = newHistories.filter((item) => item.obj === ChatRoleEnum.System);
  const chatHistories = newHistories.filter((item) => item.obj !== ChatRoleEnum.System);
  return [...systemHistories, ...chatHistories];
};
...
export const filterPublicNodeResponseData = ({
  ...
  const publicNodeMap: Record<string, any> = {
    ...
    [FlowNodeTypeEnum.bochaWebSearch]: true,
...
export const getChatSourceByPublishChannel = (publishChannel: PublishChannelEnum) => {
  switch (publishChannel) {
    ...
    case PublishChannelEnum.iframe:
      return ChatSourceEnum.share;
    // 访问控制改为 UnifiedLinkType，不再用渠道枚举表达
