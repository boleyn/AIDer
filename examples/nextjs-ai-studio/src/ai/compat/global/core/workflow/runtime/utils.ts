...
export const valueTypeFormat = (value: any, valueType?: WorkflowIOValueTypeEnum) => {
  ...
  if (
    ...
    (valueType === WorkflowIOValueTypeEnum.chatHistory &&
      (typeof value === 'number' ||
        (typeof value === 'object' && Array.isArray((value as any).histories)))) ||
  ...
  if (valueType === WorkflowIOValueTypeEnum.chatHistory) {
    if (isObjectString(value)) {
      try {
        ...
        if (
          ...
          Array.isArray((parsed as ChatHistoryWithSummaryType).histories)
        ) {
          return parsed;
        }
        return { histories: [] };
    ...
    if (typeof value === 'object' && Array.isArray((value as any).histories)) {
      return value;
    }
    return { histories: [] };
  }
  if (
    [WorkflowIOValueTypeEnum.selectApp, WorkflowIOValueTypeEnum.object].includes(valueType) &&
    typeof value === 'string'
  ) {
    if (isObjectString(value)) {
      try {
...
export const getLastInteractiveValue = (
  ...
  if (lastAIMessage) {
    ...
    // Check is outline interactive
    if (
      lastValue.interactive.type === 'outlineInteractive' &&
      !lastValue.interactive.params.action
    ) {
      return lastValue.interactive;
    }
  }
...
export const checkNodeRunStatus = ({
  ...
  const incomingEdges = runtimeEdges.filter((edge) => edge.target === node.nodeId);

  if (commonEdges.length === 0 && recursiveEdgeGroups.length === 0 && incomingEdges.length > 0) {
    if (
      incomingEdges.some((item) => item.status === 'active') &&
      incomingEdges.every((item) => item.status !== 'waiting')
    ) {
      return 'run';
    }
    if (incomingEdges.every((item) => item.status === 'skipped')) {
      return 'skip';
    }
    return 'wait';
  }
...
export const getReferenceVariableValue = ({
  ...
  if (isValidReferenceValueFormat(value)) {
    ...
    if (sourceNodeId === VARIABLE_NODE_ID) {
      ...
      console.log('[getReferenceVariableValue] Global Variable:', {
        outputId,
        value: variables[outputId],
        hasKey: outputId in variables
      });
...
/* Restore variables from interactive state */
export function restoreVariablesFromInteractive(
  currentVariables: Record<string, any>,
  lastInteractive?: InteractiveNodeResponseType
): Record<string, any> {
  // 如果交互状态中保存了变量，则恢复这些变量
  if (lastInteractive?.variables) {
    return {
      ...currentVariables,
      ...lastInteractive.variables
    };
  }
