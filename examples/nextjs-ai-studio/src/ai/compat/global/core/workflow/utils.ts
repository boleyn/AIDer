...
import { defaultWordCloud3DConfig } from '../app/constants';
...
export const getAppChatConfig = ({
  ...
  const config: AppChatConfigType = {
    ...
    instruction,
    autoExecute,
    // 添加wordCloud3DConfig的默认值处理
    wordCloud3DConfig: chatConfig?.wordCloud3DConfig ?? defaultWordCloud3DConfig,
    ...chatConfig,
    variables: mergeAndFilterVariables(storeVariables, chatConfig?.variables, variables),
...
/**
 * 合并和过滤变量，特殊处理'model_id', 'dataset', 'web'三个键
 * @param storeVariables 存储的变量
 * @param chatConfigVariables 聊天配置的变量
 * @param defaultVariables 默认变量
 * @returns 处理后的变量数组
 */
export const mergeAndFilterVariables = (
  storeVariables?: VariableItemType[],
  chatConfigVariables?: VariableItemType[],
  defaultVariables?: VariableItemType[]
): VariableItemType[] => {
  // 使用基础变量作为起点
  const baseVariables = storeVariables ?? chatConfigVariables ?? defaultVariables ?? [];

  // 如果没有chatConfig变量，直接返回基础变量
  if (!chatConfigVariables || chatConfigVariables.length === 0) return baseVariables;

  // 复制一份变量以进行修改
  const resultVariables = [...baseVariables];

  // 特殊处理的键名
  const specialKeys = ['model_id', 'dataset', 'web', 'model_list'];

  // 获取chatConfig中特殊键的项
  const specialItemsInChatConfig = specialKeys.reduce(
    (acc, key) => {
      const item = chatConfigVariables.find((v) => v.key === key);
      if (item) acc[key] = item;
      return acc;
    },
    {} as Record<string, VariableItemType>
  );

  // 1. 移除resultVariables中存在于specialKeys但不在chatConfigVariables中的项
  for (let i = resultVariables.length - 1; i >= 0; i--) {
    const key = resultVariables[i].key;
    if (specialKeys.includes(key) && !specialItemsInChatConfig[key]) {
      resultVariables.splice(i, 1);
    }
  }

  // 2. 添加或更新chatConfigVariables中的特殊项
  for (const key of specialKeys) {
    if (specialItemsInChatConfig[key]) {
      const existingIndex = resultVariables.findIndex((v) => v.key === key);
      if (existingIndex >= 0) {
        resultVariables[existingIndex] = specialItemsInChatConfig[key];
      } else {
        resultVariables.push(specialItemsInChatConfig[key]);
      }
    }
  }
