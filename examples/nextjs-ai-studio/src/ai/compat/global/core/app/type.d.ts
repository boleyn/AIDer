...
export type AppDatasetSearchParamsType = {
  ...
  datasetUsePrivateDataset?: boolean; // use private dataset
};
...
export interface PrivateDatasetConfigType {
  enabled: boolean;
}

/* app chat config type */
// 在AppChatConfigType接口中添加
export type AppChatConfigType = {
  ...
  updateConfig?: AppUpdateConfigType;
  privateDatasetConfig?: PrivateDatasetConfigType;
  // 添加3D词云配置
  wordCloud3DConfig?: AppWordCloud3DConfigType;
...
// 添加3D词云配置类型定义
export type AppWordCloud3DConfigType = {
  open: boolean;
  questions: WordCloudQuestionType[];
  autoRotate?: boolean;
  showOnStart?: boolean;
};

export type WordCloudQuestionType = {
  id: string;
  text: string;
  value: number;
  color: string;
