...
import { getWorkflowGlobalVariables, getSmartSelectedTypeIndex } from './utils';
...
export const getGlobalVariableNode = ({
  ...
  const variableNode: FlowNodeItemType = {
    ...
    outputs: globalVariables.map((item) => ({
      ...
      // 保留 parentKey 用于三级菜单组织
      parentKey: (item as any).parentKey
    }))
  };
...
export const v1Workflow2V2 = (
  ...
  const newNodes: StoreNodeItemType[] = copyNodes.map((node) => {
    ...
    const nodeTypeMap = {
      ...
      [FlowTypeEnum.taskRecognize]: FlowNodeTypeEnum.taskRecognize,
    ...
    const inputs = node.inputs
      .map<FlowNodeInputItemType>((input) => {
        ...
        // 创建临时的输入对象来进行智能判断
        const tempInput: FlowNodeInputItemType = {
          ...input,
          renderTypeList,
          value: input.value
        };
        ...
        const smartSelectedTypeIndex = getSmartSelectedTypeIndex(tempInput);

        const newInput: FlowNodeInputItemType = {
          ...input,
          selectedTypeIndex: smartSelectedTypeIndex,
          renderTypeList,
  ...
  copyNodes.forEach((node) => {
    node.outputs.forEach((output) => {
      output.targets?.forEach((target) => {
        ...
        } else if (node.flowType === FlowTypeEnum.taskRecognize) {
          newEdges.push({
            source: node.moduleId,
            sourceHandle: getHandleId(node.moduleId, 'source', output.key),
            target: target.moduleId,
