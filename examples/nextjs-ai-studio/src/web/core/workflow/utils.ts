...
/**
 * 智能判断输入应该使用哪个渲染类型的索引
 * 主要用于解决引用变量被错误识别为手动选择的问题
 */
export const getSmartSelectedTypeIndex = (
  input: FlowNodeInputItemType,
  templateInput?: FlowNodeInputItemType
): number => {
  // 如果已经有明确的 selectedTypeIndex 且不为 undefined，直接使用
  if (input.selectedTypeIndex !== undefined && input.selectedTypeIndex !== null) {
    return input.selectedTypeIndex;
  }

  // 智能判断逻辑
  const { renderTypeList, value } = input;

  // 如果只有一个渲染类型，直接返回0
  if (!renderTypeList || renderTypeList.length <= 1) {
    return 0;
  }

  // 检查是否是引用变量格式：[nodeId, outputKey]
  const isReferenceValue = (val: any): boolean => {
    return (
      Array.isArray(val) &&
      val.length === 2 &&
      typeof val[0] === 'string' &&
  ...
  // 特殊处理支持 selectDataset 和 reference 的情况
  if (
    renderTypeList.includes(FlowNodeInputTypeEnum.selectDataset) &&
    renderTypeList.includes(FlowNodeInputTypeEnum.reference)
  ) {
    // 如果value是引用格式，使用reference类型
    if (isReferenceValue(value)) {
      return renderTypeList.indexOf(FlowNodeInputTypeEnum.reference);
    }

    // 如果value是数据集选择数组格式（包含数据集对象的数组），使用selectDataset类型
    const isDatasetArray =
      Array.isArray(value) &&
      value.length > 0 &&
      value[0] &&
      typeof value[0] === 'object' &&
      (value[0].datasetId || value[0].name || value[0].avatar !== undefined);

    if (isDatasetArray) {
      return renderTypeList.indexOf(FlowNodeInputTypeEnum.selectDataset);
    }

    // 如果value是空数组，默认使用selectDataset类型（索引0）
    if (Array.isArray(value) && value.length === 0) {
      const selectDatasetIndex = renderTypeList.indexOf(FlowNodeInputTypeEnum.selectDataset);
      return selectDatasetIndex >= 0 ? selectDatasetIndex : 0;
    }
  ...
  // 通用的引用变量检查
  if (renderTypeList.includes(FlowNodeInputTypeEnum.reference) && isReferenceValue(value)) {
    return renderTypeList.indexOf(FlowNodeInputTypeEnum.reference);
  }

  // 其他情况检查：如果value是数组格式的引用
  if (
    renderTypeList.includes(FlowNodeInputTypeEnum.reference) &&
    Array.isArray(value) &&
    value.length === 2
  ...
  const nodeItem: FlowNodeItemType = {
    ...
    inputs: templateInputs
      .map<FlowNodeInputItemType>((templateInput) => {
        ...
        // 使用智能判断函数来确定正确的selectedTypeIndex
        const smartSelectedTypeIndex = getSmartSelectedTypeIndex(storeInput, templateInput);
        ...
        return {
          ...
          selectedTypeIndex: smartSelectedTypeIndex ?? templateInput.selectedTypeIndex,
...
export const filterWorkflowNodeOutputsByType = (
  ...
  const validTypeMap: Record<WorkflowIOValueTypeEnum, WorkflowIOValueTypeEnum[]> = {
    ...
    [WorkflowIOValueTypeEnum.longTextOutline]: [
      WorkflowIOValueTypeEnum.longTextOutline,
      WorkflowIOValueTypeEnum.arrayAny
    ],
    [WorkflowIOValueTypeEnum.longTextOutlineItem]: [
      WorkflowIOValueTypeEnum.longTextOutlineItem,
      WorkflowIOValueTypeEnum.arrayAny
    ],
    [WorkflowIOValueTypeEnum.longTextParagraph]: [
      WorkflowIOValueTypeEnum.longTextParagraph,
      WorkflowIOValueTypeEnum.arrayAny
    ],
    [WorkflowIOValueTypeEnum.longTextParagraphs]: [
      WorkflowIOValueTypeEnum.longTextParagraphs,
      WorkflowIOValueTypeEnum.arrayAny
    ],
...
export const checkWorkflowNodeAndConnection = ({
  ...
  for (const node of nodes) {
    ...
    const isToolNode = edges.some(
      (edge) =>
        (edge.targetHandle === NodeOutputKeyEnum.selectedTools ||
          edge.targetHandle === NodeOutputKeyEnum.taskTarget) &&
        edge.target === node.data.nodeId
    );
    ...
    if (data.flowNodeType === FlowNodeTypeEnum.userSelect) {
      ...
      // Allow empty options - they will be filtered out during runtime
    }
    ...
    if (
      inputs.some((input) => {
        ...
        // Special handling for dataset node: allow empty datasetSelectList if private dataset is enabled
        if (
          data.flowNodeType === FlowNodeTypeEnum.datasetSearchNode &&
          input.key === NodeInputKeyEnum.datasetSelectList &&
          input.required
        ) {
          const datasetUsePrivateDataset = inputs.find(
            (i) => i.key === NodeInputKeyEnum.datasetUsePrivateDataset
          )?.value;
          // If private dataset is enabled, allow empty dataset list
          if (datasetUsePrivateDataset === true) {
            return false;
          }
        }
