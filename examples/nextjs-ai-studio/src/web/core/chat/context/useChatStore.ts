...
import { shouldStartNewSessionOnEnter } from '../sessionMeta';
      ...
      immer((set, get) => ({
        ...
        setSource(e) {
          set((state) => {
            ...
            // Re-entering same source: apply "new session on next enter" policy even if chatId exists (same tab/sessionStorage).
            if (currentSource === e && state.chatId) {
              const currentChatKey = `${e}-${state.chatId}`;
              if (shouldStartNewSessionOnEnter(currentChatKey)) {
                const newId = getNanoid(24);
                state.chatId = newId;
                state.lastChatId = `${e}-${newId}`;
              }
            }

            // 首次进入 chat 页面，如果相同的 source，则恢复上一次的 chatId
            if (!state.chatId && state.lastChatId && state.lastChatId.startsWith(e)) {
              const lastChatKey = state.lastChatId;
              const lastId = lastChatKey.slice(e.length + 1);

              if (shouldStartNewSessionOnEnter(lastChatKey)) {
                const newId = getNanoid(24);
                state.chatId = newId;
                state.lastChatId = `${e}-${newId}`;
              } else {
            ...
            } else if (e !== currentSource) {
              // 来源改变，强制重置 chatId
              const newId = getNanoid(24);
              state.chatId = newId;
              state.lastChatId = `${e}-${newId}`;
            } else if (!state.chatId) {
              // Same source but no chatId stored (e.g. cleared session storage); ensure we have one.
              const newId = getNanoid(24);
              state.chatId = newId;
              state.lastChatId = `${e}-${newId}`;
            }
