...
const ChatContextProvider = ({
  ...
  const { runAsync: runClearHistories, loading: isClearingHistory } = useRequest2(
    () => {
      // 统一入口的上下文：路由为 /chat/entry 或 /chat/entrySecure
      const isEntryContext =
        typeof window !== 'undefined' &&
        (/^\/chat\/entry(\b|\/?)/.test(window.location.pathname) ||
          /^\/chat\/entrySecure(\b|\/?)/.test(window.location.pathname));
      if (isEntryContext) {
