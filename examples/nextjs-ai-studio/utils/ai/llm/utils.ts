import { countGptMessagesTokens } from '@ai/compat/common/string/tiktoken/index';
import type {
...
} from '@ai/compat/global/core/ai/type.d';
import { axios } from '@ai/compat/common/api/axios';
import { ChatCompletionRequestMessageRoleEnum } from '@ai/compat/global/core/ai/constants';
import { i18nT } from '@ai/compat/web/i18n/utils';
import { addLog } from '@ai/compat/common/system/log';
import { getImageBase64 } from '@ai/compat/common/file/image/utils';
import { getS3ChatSource } from '@ai/compat/common/s3/sources/chat';
import { isInternalAddress } from '@ai/compat/common/system/utils';
import { getErrText } from '@ai/compat/global/common/error/utils';
...
export const loadRequestMessages = async ({
  ...
  const formatAssistantItem = (item: ChatCompletionAssistantMessageParam) => {
    const hasReasoningField =
      'reasoning_content' in (item as any) || 'reasoning_text' in (item as any);
    const reasoning =
      (item as any).reasoning_content ??
      (item as any).reasoning_text ??
      (hasReasoningField ? '' : undefined);
    ...
    return {
      ...
      ...(hasReasoningField && {
        // 深度思考模型要求历史 assistant 消息包含 reasoning_content
