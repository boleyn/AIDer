...
export const appWorkflow2Form = ({
  ...
  nodes.forEach((node) => {
    if (
      ...
      defaultAppForm.dataset.rerankSimilarity =
        findInputValueByKey(node.inputs, NodeInputKeyEnum.datasetSearchRerankSimilarity) ??
        defaultAppForm.dataset.similarity;
      ...
      defaultAppForm.dataset.datasetUsePrivateDataset = findInputValueByKey(
        node.inputs,
        NodeInputKeyEnum.datasetUsePrivateDataset
      );
    } else if (node.flowNodeType === FlowNodeTypeEnum.bochaWebSearch) {
      defaultAppForm.webSearch.enabled = true;
      defaultAppForm.webSearch.searchCount =
        findInputValueByKey(node.inputs, NodeInputKeyEnum.historyMaxAmount) || 10;
    } else if (
