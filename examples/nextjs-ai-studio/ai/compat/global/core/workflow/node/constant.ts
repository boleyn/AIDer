...
export enum FlowNodeTypeEnum {
  ...
  bochaWebSearch = 'bochaWebSearch',

  answerNode = 'answerNode',
  classifyQuestion = 'classifyQuestion',
  taskRecognize = 'taskRecognize',
  ...
  dynamicUserSelect = 'dynamicUserSelect',
  sqlbotAuth = 'sqlbotAuth',
  loop = 'loop',
  loopStart = 'loopStart',
  loopEnd = 'loopEnd',
  formInput = 'formInput',
  ...
  longTextOutline = 'longTextOutline',
  longTextParagraph = 'longTextParagraph',
  progress = 'progress'
}
...
> = {
  ...
  [WorkflowIOValueTypeEnum.longTextOutline]: {
    label: 'LongText Outline[]',
    value: WorkflowIOValueTypeEnum.longTextOutline
  },
  [WorkflowIOValueTypeEnum.longTextOutlineItem]: {
    label: 'LongText OutlineItem',
    value: WorkflowIOValueTypeEnum.longTextOutlineItem
  },
  [WorkflowIOValueTypeEnum.longTextParagraph]: {
    label: 'LongText Paragraph',
    value: WorkflowIOValueTypeEnum.longTextParagraph
  },
  [WorkflowIOValueTypeEnum.longTextParagraphs]: {
    label: 'LongText Paragraphs[]',
    value: WorkflowIOValueTypeEnum.longTextParagraphs
  },
...
// longtext outline array value desc
export const longTextOutlineValueDesc = `{
  id: string; // unique id for each paragraph
  number: string; // complete section number (e.g., "1", "1.1", "2")
  title: string; // paragraph title
  level: 1 | 2; // section level (1=main section, 2=sub section)
  parentId?: string; // parent section id (optional, only for sub sections)
  description: string; // brief description of paragraph content
  order: number; // order index(start from 1)
  estimatedWords?: number; // estimated word count
  keyPoints?: string[]; // key points to cover in this paragraph
  contextBefore?: string; // relationship with previous paragraph
  contextAfter?: string; // relationship with next paragraph
}[]`;

// longtext paragraph value desc
export const longTextParagraphValueDesc = `{
  id: string; // unique id for this paragraph
  title: string; // paragraph title
  order: number; // order index(start from 1)
...
// longtext paragraphs array value desc
export const longTextParagraphsValueDesc = `{
  id: string; // unique id for this paragraph
  title: string; // paragraph title
  order: number; // order index(start from 1)
  content: string; // markdown content
...
export const NodeGradients = {
  ...
  yellow: 'linear-gradient(180deg, rgba(255, 215, 102, 0.20) 0%, rgba(255, 255, 255, 0.00) 100%)',
